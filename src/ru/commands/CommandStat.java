	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(sender.isOp()) {
			if(args.length == 1 && args[0].equals("cleanup")) {
				...
				sender.sendMessage(ChatColor.AQUA + "ќчистка окончена");
				PlayerStat.save();
				return true;
			}
			if(args.length >= 2) {
				...
				if(PlayerStat.getRegisteredPlayerNames().contains(player) || player.equals("all")) {
					...
					if(args[0].equalsIgnoreCase("delete")) {
						...
						if(args.length >= 3) {
							...
							try {
								...
								sender.sendMessage(ChatColor.RED + "Ќет статы " + ChatColor.GOLD + args[2]);
								return true;
							}
							if(args[0].equalsIgnoreCase("zero")) {
								for(String p : players) {
									for(PlayerStat stat : stats) {
										...
										sender.sendMessage(ChatColor.AQUA + stat.getName() + ChatColor.YELLOW + " сброшен дл€ " + ChatColor.GOLD + p);
									}
								}
								PlayerStat.save();
								return true;
							...
							if(args.length >= 4) {
								...
								try {
									...
									sender.sendMessage(ChatColor.RED + "Ёто не число: " + ChatColor.GOLD + args[3]);
									return true;
								}
								if(args[0].equalsIgnoreCase("set")) {
									for(String p : players) {
										for(PlayerStat stat : stats) {
											stat.setValue(p, c);
											sender.sendMessage(
													ChatColor.AQUA + stat.getName() + ChatColor.YELLOW + " установлены на " + ChatColor.DARK_AQUA + c + ChatColor.YELLOW + " дл€ "
															+ ChatColor.GOLD + p);
										}
									}
									PlayerStat.save();
								...
								if(args[0].equalsIgnoreCase("add")) {
									for(String p : players) {
										for(PlayerStat stat : stats) {
											...
											sender.sendMessage(
													ChatColor.AQUA + stat.getName() + ChatColor.YELLOW + " теперь равны " + ChatColor.DARK_AQUA + newVal + ChatColor.YELLOW + " дл€ "
															+ ChatColor.GOLD + p);
										}
									}
									PlayerStat.save();
					...
					sender.sendMessage(ChatColor.RED + "»грока " + ChatColor.GOLD + args[1] + " не существует");
					return true;
				}
			}
		}
